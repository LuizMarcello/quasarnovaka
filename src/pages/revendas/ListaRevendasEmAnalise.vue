...
export default defineComponent({
  ...
  setup() {
    ...
    const $q = useQuasar();
    const { remove } = useApi();
    const { emanalise } = useApiRevendas();
    const { notifyError, notifySuccess } = useNotify;

    const handleListRevendasEmAnalise = async () => {
      try {
        loading.value = true;
        revendasEmAnalise.value = await emanalise("revendas");
        loading.value = false;
      } catch (error) {
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          revendasEmAnalise.value.length / initialPagination.value.rowPerPage
        )
      ),
    };
  },
