...
export default defineComponent({
  ...
  setup() {
    ...
    const $q = useQuasar();
    const { remove } = useApi();
    const { listareprovada } = useApiRevendas();
    const { notifyError, notifySuccess } = useNotify;

    const handleListRevendasAprovadas = async () => {
      try {
        loading.value = true;
        revendasReprovadas.value = await listareprovada("revendas");
        loading.value = false;
      } catch (error) {
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          revendasReprovadas.value.length / initialPagination.value.rowPerPage
        )
      ),
    };
  },
