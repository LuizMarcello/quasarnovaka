...
export default defineComponent({
  ...
  setup() {
    const revendasComPendencias = ref([]);
    ...
    const $q = useQuasar();
    const { remove } = useApi();
    const { compendencias } = useApiRevendas();
    const { notifyError, notifySuccess } = useNotify;

    const handleListRevendasAprovadas = async () => {
      try {
        loading.value = true;
        revendasComPendencias.value = await compendencias("revendas");
        loading.value = false;
      } catch (error) {
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          revendasComPendencias.value.length / initialPagination.value.rowPerPage
        )
      ),
    };
  },
