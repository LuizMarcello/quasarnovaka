...
export default defineComponent({
  ...
  setup(props) {
    ...
    const filter = ref("");
    const $q = useQuasar();
    const { handleListRoteador, remove } = useApi();
    const { notifyError, notifySuccess } = useNotify;

    const handleListEstoqueRoteador = async () => {
      try {
        loading.value = true;
        estoqueRoteador.value = await handleListRoteador("estoque");
        loading.value = false;
      } catch (error) {
    ...
    const handleEdit = (estoqueRoteador) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({ name: "form-estoque", params: { id: estoqueRoteador.id } });
    };

    const handleDetails = (estoqueRoteador) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
        name: "form-estoque-detalhes",
        params: { id: estoqueRoteador.id },
      });
    };

    const handleHistorico = (estoqueRoteador) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
    ...
    const handleRemoveEstoque = async (estoqueRoteador) => {
      try {
        $q.dialog({
          title: "Confirm",
          message: `Tem certeza que quer deletar ${estoqueRoteador.marca} ?`,
          cancel: true,
          persistent: true,
        }).onOk(async () => {
          await remove("estoque", estoqueRoteador.id);
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          estoqueRoteador.value.length / initialPagination.value.rowPerPage
        )
      ),
    };
  },
