...
export default defineComponent({
  ...
  setup(props) {
    ...
    const filter = ref("");
    const $q = useQuasar();
    const { handleListLnb, remove } = useApi();
    const { notifyError, notifySuccess } = useNotify;

    const handleListEstoqueLnb= async () => {
      try {
        loading.value = true;
        estoqueLnb.value = await handleListLnb("estoque");
        loading.value = false;
      } catch (error) {
    ...
    const handleEdit = (estoqueLnb) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({ name: "form-estoque", params: { id: estoqueLnb.id } });
    };

    const handleDetails = (estoqueLnb) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
        name: "form-estoque-detalhes",
        params: { id: estoqueLnb.id },
      });
    };

    const handleHistorico = (estoqueLnb) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
    ...
    const handleRemoveEstoque = async (estoqueLnb) => {
      try {
        $q.dialog({
          title: "Confirm",
          message: `Tem certeza que quer deletar ${estoqueLnb.marca} ?`,
          cancel: true,
          persistent: true,
        }).onOk(async () => {
          await remove("estoque", estoqueLnb.id);
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(estoqueLnb.value.length / initialPagination.value.rowPerPage)
      ),
    };
  },
});
