...
export default defineComponent({
  ...
  setup(props) {
    ...
    const filter = ref("");
    const $q = useQuasar();
    const { handleListEtria, remove } = useApi();
    const { notifyError, notifySuccess } = useNotify;

    const handleListEstoqueEtria = async () => {
      try {
        loading.value = true;
        estoqueEtria.value = await handleListEtria("estoque");
        loading.value = false;
      } catch (error) {
    ...
    const handleEdit = (estoqueEtria) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({ name: "form-estoque", params: { id: estoqueEtria.id } });
    };

    const handleDetails = (estoqueEtria) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
        name: "form-estoque-detalhes",
        params: { id: estoqueEtria.id },
      });
    };

    const handleHistorico = (estoqueEtria) => {
      /*   router.push({ name: "form-revendas", params: { id: revenda.id } }); */
      router.push({
    ...
    const handleRemoveEstoque = async (estoqueEtria) => {
      try {
        $q.dialog({
          title: "Confirm",
          message: `Tem certeza que quer deletar ${estoqueEtria.marca} ?`,
          cancel: true,
          persistent: true,
        }).onOk(async () => {
          await remove("estoque", estoqueEtria.id);
    ...
    return {
      ...
      pagesNumber: computed(() =>
        Math.ceil(
          estoqueEtria.value.length / initialPagination.value.rowPerPage
        )
      ),
    };
  },
